...
DEFAULT_LEAD_FIELDS = [
    ...
    {'key': 'date', 'label': 'Date', 'type': 'text', 'required': False, 'order': 10},  # Display only; created_at
]
...
CUSTOM_DATA_KEYS = {'address', 'product_name', 'product_count', 'product_price'}
# Keys that are display-only / system-managed (not editable in add form/public form/template)
# - date   -> created_at timestamp
# - status -> defaults to 'new', updated via status change flow
DISPLAY_ONLY_KEYS = {'date', 'status'}
